...
function getApiImageCapabilityTag(api) {
  return api?.xaiImageEnabled ? ' · xAI图像' : '';
}

function hasUsableXaiImageNode(apiNode, mode) {
  if (!apiNode?.xaiImageEnabled) return false;
  if (!String(apiNode?.apiKey || '').trim()) return false;
  if (mode === 'edit') return !!String(apiNode?.xaiImageEditUrl || '').trim();
  return !!String(apiNode?.xaiImageGenerationUrl || '').trim();
}

function resolveXaiImageNode(session, mode) {
  const preferredIndex = session?.isDirect ? (state.runtimeConfig.activeApiIndex || 0) : getGroupApiIndex(session?.channelId);
  const tried = new Set();
  const candidateIndices = [preferredIndex, ...state.apiList.map((_, index) => index)];

  for (const index of candidateIndices) {
    if (!Number.isInteger(index) || index < 0 || index >= state.apiList.length) continue;
...
async function sendXaiGeneratedImages(session, images) {
  for (const image of images) {
    if (image?.type === 'base64' && image?.data) {
      await session.send(h('image', { src: `base64://${image.data}`, subType: 1, summary: '[AI生成图片]' }));
      continue;
    }
    if (image?.type === 'url' && image?.url) {
      await session.send(h('image', { src: image.url, summary: '[AI生成图片]' }));
    }
  }
}
...
const HELP_MENU_DATA = {
  ...
  sections: [
    ...
    },
    {
      title: '图像工具',
      items: [
        { title: 'neko.生图 【提示词】', description: '调用 xAI 图像生成接口，使用已启用图像能力的节点' },
        { title: 'neko.修图 【提示词】', description: '附带图片或引用带图消息后，调用 xAI 图像编辑接口' },
      ],
    },
    {
      title: '人格管理',
...
━━ 图像工具 ━━
neko.生图 【提示词】 — 调用 xAI 图像生成接口
neko.修图 【提示词】 — 附带图片或引用带图消息后调用 xAI 图像编辑接口
...
function buildModelListFallbackText(page, paged = false) {
  if (!paged) {
    ...
    state.apiList.forEach((api, index) => {
      ...
      res += `\n[${index}] ${api.remark} ${typeDisplay}${active}\n - 模型: ${api.modelName}${getApiImageCapabilityTag(api)}`;
    });
    return res;
  }
    ...
    return await renderModelListCard(ctx, {
      ...
      items: info.items.map((api, idx) => {
        ...
        return {
          ...
          description: `${(api.aiType || 'openai').toUpperCase()} · ${api.modelName}${getApiImageCapabilityTag(api)}`,
...
function registerCommands(ctx) {
  ...
  ctx.command("neko.生图 <prompt:text>", "调用 xAI 图像生成接口生成图片")
    ...
    .action(async ({ session }, prompt) => {
      ...
      if (!cleanPrompt) return "❌ 用法：neko.生图 【提示词】";

      const { apiNode, index, usedFallback } = resolveXaiImageNode(session, 'generate');
      if (!apiNode) {
        return "❌ 未找到可用的 xAI 图像生成节点。请在 API 节点里启用 xAI 图像能力，并填写图像生成 URL。";
      }

      await session.send(`🎨 正在生成图片，使用节点 #${index} ${apiNode.remark || apiNode.modelName}。`);
      try {
        const result = await generateXaiImages(ctx, apiNode, cleanPrompt);
        await sendXaiGeneratedImages(session, result.images);
        await session.send(`✅ 生图完成：节点 #${index} · ${result.model || apiNode.xaiImageModel || 'grok-imagine-image'}${usedFallback ? '（当前聊天节点未启用图像能力，已自动改用首个可用图像节点）' : ''}`);
        return;
      } catch (e) {
        logger.warn(`xAI 生图失败 [#${index} ${apiNode.remark || apiNode.modelName}]: ${e.message}`);
        return `❌ 生图失败：${e.message || '未知错误'}`;
      }
    });
  ...
  ctx.command("neko.修图 <prompt:text>", "附带图片或引用带图消息后，调用 xAI 图像编辑接口")
    .action(async ({ session }, prompt) => {
      if (!isMaster(session)) return "❌ 权限不足，仅主人可用。";
      const cleanPrompt = String(prompt || '').trim();
      if (!cleanPrompt) return "❌ 用法：neko.修图 【提示词】";

      const imageBundle = await collectXaiEditImages(ctx, session);
      if (imageBundle.images.length === 0) {
        return "❌ 未检测到可用于修图的图片。请在命令消息里附带图片，或引用一条带图消息后再执行。";
      }

      const { apiNode, index, usedFallback } = resolveXaiImageNode(session, 'edit');
      if (!apiNode) {
        return "❌ 未找到可用的 xAI 图像编辑节点。请在 API 节点里启用 xAI 图像能力，并填写图像编辑 URL。";
      }

      await session.send(`🖌️ 正在修图，使用节点 #${index} ${apiNode.remark || apiNode.modelName}，输入图片 ${imageBundle.images.length} 张。`);
      try {
        const result = await generateXaiImages(ctx, apiNode, cleanPrompt, { images: imageBundle.images });
        await sendXaiGeneratedImages(session, result.images);
        await session.send(`✅ 修图完成：节点 #${index} · ${result.model || apiNode.xaiImageModel || 'grok-imagine-image'}${usedFallback ? '（当前聊天节点未启用图像能力，已自动改用首个可用图像节点）' : ''}`);
        return;
      } catch (e) {
        logger.warn(`xAI 修图失败 [#${index} ${apiNode.remark || apiNode.modelName}]: ${e.message}`);
        return `❌ 修图失败：${e.message || '未知错误'}`;
      }
    });
