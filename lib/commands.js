const { redactForChat } = require('./chat-error');
/**
 * commands.js — 所有指令注册
...
const { generateImages, getImageApiTimeoutMs, getImageEndpointUrls } = require('./api');
...
function getDefaultImageModelName(apiNode) {
  return getImageProviderType(apiNode) === 'openai' ? 'gpt-image-2' : 'grok-imagine-image';
}

function getImageProviderLabel(apiNode) {
  return getImageProviderType(apiNode) === 'openai' ? 'OpenAI' : 'xAI';
}

function imageNodeSupportsEdit(apiNode) {
  const provider = getImageProviderType(apiNode);
  const explicit = readBooleanConfig(apiNode?.supportsEdit);
  if (explicit !== undefined) return explicit;
  return provider === 'xai' || (provider === 'openai' && isGptImage2Model(apiNode?.modelName || getDefaultImageModelName(apiNode)));
}

function getImageNodeCapabilityLabel(apiNode) {
  return imageNodeSupportsEdit(apiNode) ? '生图+修图' : '仅生图';
}

function isGptImage2Node(apiNode) {
  return getImageProviderType(apiNode) === 'openai' && isGptImage2Model(apiNode?.modelName || getDefaultImageModelName(apiNode));
}
...
function buildImageNodeLabel(apiNode, index) {
  return `#${index} ${apiNode?.remark || apiNode?.modelName || getDefaultImageModelName(apiNode) || '未命名节点'}`;
}
...
function buildImageModelFallbackNotice(result) {
  const fallback = result?.modelFallback;
  if (!fallback?.from || !fallback?.to) return '';
  return `\n⚠️ xAI 图像模型 ${fallback.from} 当前暂时不可用，已自动回退到 ${fallback.to}。`;
}

function buildImageEndpointFallbackNotice(result) {
  const fallback = result?.endpointFallback;
  if (!fallback || !Number.isInteger(Number(fallback.usedIndex)) || !Number.isInteger(Number(fallback.total))) return '';
  return `\n⚠️ 主图像接口 URL 调用失败，已自动改用第 ${Number(fallback.usedIndex) + 1}/${Number(fallback.total)} 个备用 URL。`;
}

function buildImageStreamingNotice(result) {
  const streaming = result?.imageStreaming;
  if (!streaming) return '';
  if (streaming.used) {
    return `\nℹ️ 本次使用 OpenAI 图像流式请求，收到中间图 ${Number(streaming.partialCount) || 0} 张。`;
  }
  if (streaming.fallback) {
    return `\n⚠️ OpenAI 图像流式请求失败，已自动回退普通请求。`;
  }
  if (streaming.ignoredByProvider) {
    return `\n⚠️ 当前节点未返回标准流式响应，已按普通图像响应处理。`;
  }
  return '';
}

function formatImageTimeoutHint() {
  const timeoutMs = getImageApiTimeoutMs();
  const seconds = Math.max(1, Math.round(timeoutMs / 1000));
  if (seconds >= 60) {
    const minutes = seconds / 60;
    const label = Number.isInteger(minutes) ? `${minutes}分钟` : `${minutes.toFixed(1)}分钟`;
    return `最长等待约 ${label}`;
  }
  return `最长等待约 ${seconds}秒`;
}
...
async function sendAutoRecallImageNotice(session, text, delayMs = IMAGE_NOTICE_RECALL_MS) {
  const messageIds = await session.send(redactForChat(text, [...(state.apiList || []), ...(state.imageApiList || [])]));
...
function buildXaiGenerateUsageText() {
  return `❌ 用法：neko.生图 【提示词】 [--count 1-10] [--ratio 16:9] [--resolution 2k] [--model gpt-image-2] [--node 0]
示例1：neko.生图 赛博朋克夜雨里的猫娘 --ratio 16:9 --resolution 2k
示例2：neko.生图 Q版白发少女贴纸 --count 2 --ratio 1:1
示例3：引用一条带图消息，再发送：neko.生图 按这张图的角色画成水彩插画
说明：
${buildXaiImageOptionHintText()}
如果不想手动指定宽高比或分辨率，直接省略 --ratio / --resolution，命令会走当前图像节点里保存的默认值。
--count 会拆成连续的单张请求：上一张成功返回并发送后，才会自动继续下一张；某一轮全部路由节点失败后会停止。
当当前节点是 gpt-image-2 且配置了修图 URL / supportsEdit 时，可以直接引用带图消息进行 neko.生图 提示词，把图片作为参考图来生成。`;
}
...
说明：
${buildXaiImageOptionHintText()}
修图命令需要当前消息附图，或引用一条带图消息。gpt-image-2 图像节点在 supportsEdit 开启且填写修图 URL 时也可以处理参考图；若不写 --ratio / --resolution，则优先走当前图像节点的默认值。`;
}
...
function hasUsableXaiImageNode(apiNode, mode) {
  const provider = getImageProviderType(apiNode);
  if (!['xai', 'openai'].includes(provider)) return false;
  if (!String(apiNode?.apiKey || '').trim()) return false;
  if (mode === 'edit') return imageNodeSupportsEdit(apiNode) && getImageEndpointUrls(apiNode, 'edit').length > 0;
  return getImageEndpointUrls(apiNode, 'generate').length > 0;
}
...
function normalizeImageRouteOrder(value) {
...
function getImageRouterConfig() {
  const raw = state.runtimeConfig.imageRouter && typeof state.runtimeConfig.imageRouter === 'object'
    ? state.runtimeConfig.imageRouter
    : {};
  return {
    enabled: raw.enabled === true,
    order: normalizeImageRouteOrder(raw.order),
  };
}

function resolveXaiImageRoute(session, mode, preferredOverride) {
  const router = getImageRouterConfig();
  const hasPreferredOverride = Number.isInteger(preferredOverride);
  const preferredIndex = hasPreferredOverride ? preferredOverride : (Number(state.runtimeConfig.activeImageApiIndex) || 0);
  const baseOrder = router.enabled && router.order.length > 0
    ? router.order
    : [preferredIndex, ...state.imageApiList.map((_, index) => index)];
  const candidateIndices = hasPreferredOverride && router.enabled && router.order.length > 0
    ? [preferredIndex, ...baseOrder.filter((index) => index !== preferredIndex)]
    : baseOrder;
  ...
  const skipped = [];
  for (const index of candidateIndices) {
    if (!Number.isInteger(index) || tried.has(index)) continue;
    tried.add(index);
    if (index < 0 || index >= state.imageApiList.length) {
      skipped.push({ index, reason: '节点编号超出范围' });
      continue;
    }
    const apiNode = state.imageApiList[index];
    if (!hasUsableXaiImageNode(apiNode, mode)) {
      skipped.push({ index, reason: mode === 'edit' ? '节点不支持修图或缺少修图 URL/API Key' : '节点缺少生图 URL/API Key' });
      continue;
    }
    route.push({ apiNode, index });
    if (!router.enabled || router.order.length === 0) break;
  }

  if (router.enabled && router.order.length > 0) {
    logger.info(`[图像路由集群] 已启用，模式=${mode === 'edit' ? '修图/参考图' : '生图'} 配置顺序=[${router.order.join(' → ')}] 实际队列=[${route.map((item) => item.index).join(' → ') || '空'}]${skipped.length ? ` 跳过=${skipped.map((item) => `#${item.index}:${item.reason}`).join(' | ')}` : ''}`);
  }

  const first = route[0] || null;
  return {
    route,
    apiNode: first?.apiNode || null,
    index: first?.index ?? -1,
    preferredIndex,
    usedFallback: !!first && first.index !== preferredIndex,
    routerEnabled: router.enabled && router.order.length > 0,
    configuredOrder: router.order,
    skipped,
  };
}

function buildImageRouteLabel(routePlan) {
  const route = Array.isArray(routePlan?.route) ? routePlan.route : [];
  if (route.length === 0) return '[无可用图像节点]';
  return route.map(({ apiNode, index }) => buildImageNodeLabel(apiNode, index)).join(' → ');
}

function buildImageRouteFallbackNotice(result) {
  const route = result?.imageRoute;
  if (!route || Number(route.usedRouteIndex || 0) <= 0) return '';
  const failedCount = Array.isArray(route.failures) ? route.failures.length : Number(route.usedRouteIndex || 0);
  return `\n⚠️ 图像路由集群：前 ${failedCount} 个节点失败，已自动切换到 ${route.usedNodeLabel} 并成功返回结果。`;
}

function buildImageRouteFailureMessage(failures) {
  const list = Array.isArray(failures) ? failures : [];
  if (list.length === 0) return '图像路由集群没有可尝试的图像节点';
  const last = list[list.length - 1];
  const detail = list
    .map((item) => `${item.label || `#${item.index}`} · ${item.modelName || 'Unknown model'}: ${item.error || '未知错误'}`)
    .join('；');
  return `图像路由集群全部 ${list.length} 个节点均失败；最后错误：${last?.error || '未知错误'}。失败路径：${detail}`;
}

function filterSendableGeneratedImages(images) {
  return Array.isArray(images)
    ? images.filter(image => (image?.type === 'base64' && image?.data) || (image?.type === 'url' && image?.url))
    : [];
}

function cloneImageRouteOptions(options = {}) {
  const cloned = { ...options };
  if (Array.isArray(options.images)) {
    cloned.images = options.images.map((image) => (
...
async function generateImagesWithRoute(ctx, routePlan, prompt, options = {}, meta = {}) {
  const route = Array.isArray(routePlan?.route) ? routePlan.route : [];
  if (route.length === 0) {
    throw new Error('图像路由集群没有可尝试的图像节点');
  }

  const failures = [];
  const routePrompt = String(prompt || '');
  for (let routeIndex = 0; routeIndex < route.length; routeIndex++) {
    const { apiNode, index } = route[routeIndex];
    const nodeLabel = buildImageNodeLabel(apiNode, index);
    const requestOptions = cloneImageRouteOptions(options);
    const sourceImageCount = Array.isArray(requestOptions.images) ? requestOptions.images.length : 0;
    logger.info(`[图像路由集群] ${meta.actionLabel || '图像任务'} 尝试 ${routeIndex + 1}/${route.length} 节点=${nodeLabel} 提示词长度=${routePrompt.trim().length} 参考图=${sourceImageCount}`);
    try {
      const result = await generateImages(ctx, apiNode, routePrompt, requestOptions);
      logger.info(`[图像路由集群] ${meta.actionLabel || '图像任务'} 节点成功 节点=${nodeLabel} 已失败节点=${routeIndex}`);
      return {
        ...result,
        routeNode: { index, apiNode },
        imageRoute: {
          enabled: routePlan.routerEnabled === true,
          usedRouteIndex: routeIndex,
          total: route.length,
          usedNodeIndex: index,
          usedNodeLabel: nodeLabel,
          path: route.map((item) => buildImageNodeLabel(item.apiNode, item.index)),
          failures,
        },
      };
    } catch (error) {
      const errMsg = error?.message || '未知错误';
      failures.push({ index, label: nodeLabel, modelName: apiNode.modelName || getDefaultImageModelName(apiNode), error: errMsg });
      logger.warn(`[图像路由集群] ${meta.actionLabel || '图像任务'} 节点失败 ${routeIndex + 1}/${route.length} 节点=${nodeLabel} 错误=${errMsg}${routeIndex < route.length - 1 ? '，继续尝试下一个节点' : '，路由已耗尽'}`);
    }
  }

  if (route.length === 1 && failures.length === 1) {
    throw new Error(`${failures[0].modelName || 'Unknown model'}: ${failures[0].error || '图像请求失败'}`);
  }
  throw new Error(buildImageRouteFailureMessage(failures));
}
...
async function collectImageReferenceInputs(ctx, session) {
  return collectXaiEditImages(ctx, session);
}

async function sendXaiGeneratedImages(session, images, meta = {}) {
  const validImages = filterSendableGeneratedImages(images);
  ...
  sections: [
    ...
    },
    {
      ...
      items: [
        ...
        { title: 'neko.生图 【提示词】 --ratio 16:9 --resolution 2k', description: '图像节点生图；支持引用带图消息作为参考图，比例冒号请用英文 ":"' },
        { title: 'neko.修图 【提示词】 --ratio 1:1 --resolution 1k', description: '图像节点修图；图片来自当前消息或引用，比例与清晰度不写则走节点默认值' },
...
function buildImageModelListFallbackText(page) {
  ...
  info.items.forEach((api, idx) => {
    ...
    const provider = getImageProviderLabel(api);
    res += `\n[${realIndex}] ${api.remark || '无备注'} [${provider} / ${getImageNodeCapabilityLabel(api)}]${active}\n - 模型: ${api.modelName || getDefaultImageModelName(api)}`;
  });
  if (info.totalPages > 1) {
...
function buildImageModelSearchResultText(keyword, page) {
  ...
  const results = state.imageApiList
    ...
    .filter(({ api }) => {
      ...
      const capability = getImageNodeCapabilityLabel(api).toLowerCase();
      return provider.includes(normalizedKeyword)
        ...
        || capability.includes(normalizedKeyword);
    });
  ...
  pageResults.forEach(({ api, index }) => {
    ...
    const provider = getImageProviderLabel(api);
    res += `\n[${index}] ${api.remark || '无备注'} [${provider} / ${getImageNodeCapabilityLabel(api)}]${active}\n - 模型: ${api.modelName || getDefaultImageModelName(api)}`;
  });
...
function registerCommands(ctx) {
  ...
  ctx.command("neko.图像模型切换 <index:number>", "切换当前使用的图像API节点")
    .action(async ({ session }, index) => {
      ...
      saveRuntimeConfig();
      const currentImageNode = state.imageApiList[index];
      return `✅ 已成功切换至图像节点 [${index}] (${currentImageNode.remark})\n当前图像模型：${currentImageNode.modelName || getDefaultImageModelName(currentImageNode)}\n节点能力：${getImageNodeCapabilityLabel(currentImageNode)}`;
    });

  ctx.command("neko.生图 <prompt:text>", "调用图像生成节点生成图片")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      const initialRoutePlan = resolveXaiImageRoute(session, 'generate', requestedNode);
      if (!initialRoutePlan.apiNode) {
        ...
        return "❌ 未找到可用的图像生成节点。请在 image_api_config.json 或 GUI 的图像节点列表中配置图像生成节点。";
      }

      const referenceBundle = initialRoutePlan.route.some(({ apiNode }) => imageNodeSupportsEdit(apiNode))
        ? await collectImageReferenceInputs(ctx, session)
        : { images: [], currentCount: 0, quotedCount: 0 };
      const referenceImages = referenceBundle.images || [];
      const hasReferenceImages = referenceImages.length > 0;
      const routePlan = hasReferenceImages ? resolveXaiImageRoute(session, 'edit', requestedNode) : initialRoutePlan;
      const { apiNode, index, usedFallback } = routePlan;
      if (!apiNode) {
        logger.warn(`[图像任务] 生图参考图请求未找到可用修图节点 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 提示词预览=${buildImagePromptPreview(cleanPrompt)} 输入图=${referenceImages.length}`);
        appendUsageEvent(session, {
          category: 'image',
          action: 'generate',
          allowed: false,
          ...
          reason: 'no-node',
          isMasterUser: access.isMasterUser === true,
          detail: { sourceImageCount: referenceImages.length },
        });
        return "❌ 检测到参考图，但没有可用的图像编辑/参考图节点。请在 GUI 的图像节点列表中配置支持修图的节点。";
      }

      const nodeLabel = buildImageNodeLabel(apiNode, index);
      const routeLabel = buildImageRouteLabel(routePlan);
      logger.info(`[图像任务] 收到生图请求 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} fallback=${usedFallback ? '是' : '否'} 图像路由=${routePlan.routerEnabled ? routeLabel : '未启用'} 提示词长度=${cleanPrompt.length} 提示词预览=${buildImagePromptPreview(cleanPrompt)} 选项=${buildImageOptionSummary(options, { requestedCount: imageAmount, sourceImageCount: referenceImages.length, currentImageCount: referenceBundle.currentCount, quotedImageCount: referenceBundle.quotedCount })}`);
      ...
      const referenceNotice = hasReferenceImages
        ? `\n已检测到参考图 ${referenceImages.length} 张，将根据提示词参考生成。`
        : '';
      const gptImage2Notice = isGptImage2Node(apiNode)
        ? `\n正在调用 gpt-image-2。该模型可以直接带图片参考来进行生图，直接引用带图消息后发送 neko.生图 提示词 即可。`
        : '';
      const routeWorkingNotice = routePlan.routerEnabled && routePlan.route.length > 1
        ? `图像路由集群：${routeLabel}`
        : `使用节点 #${index} ${apiNode.remark || apiNode.modelName}`;
      await sendAutoRecallImageNotice(session, `🎨 正在生成图片，${routeWorkingNotice}。${referenceNotice}${gptImage2Notice}\n${formatImageTimeoutHint()}，可在图像节点管理器调整超时时间。\n${IMAGE_NOTICE_RECALL_HINT}`);
      let completedRounds = 0;
      let totalReturnedCount = 0;
      let totalSentCount = 0;
      let totalSendFailedCount = 0;
      let totalAcklessCount = 0;
      let finalQuotaUsage = null;
      let lastSuccessResult = null;
      let lastSuccessNodeIndex = index;
      let lastSuccessNode = apiNode;
      let lastSuccessNodeLabel = nodeLabel;
      let routeFallbackCount = 0;
      let modelFallbackCount = 0;
      let endpointFallbackCount = 0;
      try {
        for (let round = 1; round <= imageAmount; round++) {
          logger.info(`[图像任务] 生图连续请求开始 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 进度=${round}/${imageAmount} 路由=${routePlan.routerEnabled ? routeLabel : '未启用'} 提示词预览=${buildImagePromptPreview(cleanPrompt)}`);
          const result = await generateImagesWithRoute(ctx, routePlan, cleanPrompt, {
            n: 1,
            aspectRatio: options?.ratio,
            resolution: options?.resolution,
            model: options?.model,
            images: hasReferenceImages ? referenceImages : undefined,
          }, { actionLabel: `生图 ${round}/${imageAmount}` });
          const successNodeIndex = Number.isInteger(Number(result?.routeNode?.index)) ? Number(result.routeNode.index) : index;
          const successNode = result?.routeNode?.apiNode || state.imageApiList[successNodeIndex] || apiNode;
          const successNodeLabel = buildImageNodeLabel(successNode, successNodeIndex);
          const sendableImages = filterSendableGeneratedImages(result.images).slice(0, 1);
          if (sendableImages.length === 0) {
            throw new Error(`第 ${round}/${imageAmount} 张生成结果为空或不可发送`);
          }

          totalReturnedCount += Array.isArray(result.images) ? result.images.length : 0;
          completedRounds++;
          lastSuccessResult = result;
          lastSuccessNodeIndex = successNodeIndex;
          lastSuccessNode = successNode;
          lastSuccessNodeLabel = successNodeLabel;
          if (Number(result?.imageRoute?.usedRouteIndex || 0) > 0) routeFallbackCount++;
          if (result?.modelFallback) modelFallbackCount++;
          if (result?.endpointFallback) endpointFallbackCount++;

          finalQuotaUsage = recordImageQuotaUsage(session, 'generate', 1, access.isMasterUser, {
            modelName: result.model || successNode.modelName || getDefaultImageModelName(successNode),
            nodeRemark: successNode.remark || successNode.modelName,
            detail: {
              requestedAmount: imageAmount,
              currentRound: round,
              returnedAmount: Array.isArray(result.images) ? result.images.length : 0,
              sourceImageCount: referenceImages.length,
              routePath: result?.imageRoute?.path,
              failedRouteNodes: result?.imageRoute?.failures,
            },
          });
          const sendStats = await sendXaiGeneratedImages(session, sendableImages, { actionLabel: `生图结果 ${round}/${imageAmount}`, nodeLabel: successNodeLabel });
          totalSentCount += sendStats.sentCount;
          totalSendFailedCount += sendStats.failedCount;
          totalAcklessCount += sendStats.acklessCount;
          logger.info(`[图像任务] 生图连续请求完成 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 进度=${round}/${imageAmount} 节点=${successNodeLabel} 返回张数=${Array.isArray(result.images) ? result.images.length : 0} 已发送=${sendStats.sentCount} 发送失败=${sendStats.failedCount}`);
          if (round < imageAmount) await sleep(650);
        }

        const modelFallbackNotice = modelFallbackCount > 0
          ? `\n⚠️ 连续生图中有 ${modelFallbackCount} 次触发模型回退，最后成功模型：${lastSuccessResult?.model || lastSuccessNode.modelName || getDefaultImageModelName(lastSuccessNode)}。`
          : '';
        const endpointFallbackNotice = endpointFallbackCount > 0
          ? `\n⚠️ 连续生图中有 ${endpointFallbackCount} 次触发同节点 URL 备用切换。`
          : '';
        const streamingNotice = buildImageStreamingNotice(lastSuccessResult);
        const routeFallbackNotice = routeFallbackCount > 0
          ? `\n⚠️ 连续生图中有 ${routeFallbackCount} 次触发图像路由节点切换，已由后续节点成功接力。`
          : '';
        const sendNotice = totalSendFailedCount > 0
          ? `\n⚠️ 实际发送成功 ${totalSentCount} 张，发送失败 ${totalSendFailedCount} 张。`
          : '';
        logger.info(`[图像任务] 生图完成 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${lastSuccessNodeLabel} 路由=${routePlan.routerEnabled ? routeLabel : '未启用'} 请求张数=${imageAmount} 完成轮次=${completedRounds} 原始返回张数=${totalReturnedCount} 已发送=${totalSentCount} 发送失败=${totalSendFailedCount} 无ACK=${totalAcklessCount} 模型=${lastSuccessResult?.model || lastSuccessNode.modelName || getDefaultImageModelName(lastSuccessNode)} 路由回退次数=${routeFallbackCount} 模型回退次数=${modelFallbackCount} URL回退次数=${endpointFallbackCount}`);
        await sendAutoRecallImageNotice(session, `✅ 生图完成：已连续生成 ${completedRounds}/${imageAmount} 张，最后节点 #${lastSuccessNodeIndex} · ${lastSuccessResult?.model || lastSuccessNode.modelName || getDefaultImageModelName(lastSuccessNode)}${usedFallback && !routePlan.routerEnabled ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${routeFallbackNotice}${modelFallbackNotice}${endpointFallbackNotice}${streamingNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...finalQuotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
        logger.warn(`[图像任务] 生图失败 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} 路由=${routePlan.routerEnabled ? routeLabel : '未启用'} 已完成=${completedRounds}/${imageAmount} 提示词长度=${cleanPrompt.length} 提示词预览=${buildImagePromptPreview(cleanPrompt)} 错误=${e.message || '未知错误'}`);
        appendUsageEvent(session, {
          category: 'image',
          action: 'generate',
          allowed: false,
          amount: Math.max(imageAmount - completedRounds, 1),
          reason: 'request-failed',
          isMasterUser: access.isMasterUser === true,
          modelName: apiNode.modelName,
          nodeRemark: apiNode.remark,
          detail: { error: e.message || '未知错误', requestedAmount: imageAmount, completedAmount: completedRounds, routePath: routePlan.route.map((item) => buildImageNodeLabel(item.apiNode, item.index)) },
        });
        if (completedRounds > 0) {
          const sendNotice = totalSendFailedCount > 0
            ? `\n⚠️ 已发送成功 ${totalSentCount} 张，发送失败 ${totalSendFailedCount} 张。`
            : '';
          await sendAutoRecallImageNotice(session, `⚠️ 连续生图中断：已成功生成 ${completedRounds}/${imageAmount} 张；第 ${completedRounds + 1} 张在所有可用路由节点均失败后停止。\n失败原因：${e.message || '未知错误'}${sendNotice}\n${finalQuotaUsage ? buildImageQuotaUsageNotice({ ...finalQuotaUsage, isMasterUser: access.isMasterUser }) + '\n' : ''}${IMAGE_NOTICE_RECALL_HINT}`);
        } else {
  ...
  ctx.command("neko.修图 <prompt:text>", "附带图片或引用带图消息后，调用图像编辑节点")
    .option('ratio', '--ratio <ratio:string>')
    .option('resolution', '--resolution <resolution:string>')
    .option('model', '--model <model:string>')
    .option('node', '--node <node:number>')
    .action(async ({ session, options }, prompt) => {
      ...
      const requestedNode = Number.isInteger(Number(options?.node)) ? Number(options.node) : undefined;
      const routePlan = resolveXaiImageRoute(session, 'edit', requestedNode);
      const { apiNode, index, usedFallback } = routePlan;
      if (!apiNode) {
        ...
        return "❌ 未找到可用的图像编辑节点。请在 image_api_config.json 或 GUI 的图像节点列表中配置支持修图的图像节点。gpt-image-2 若要处理引用图，请确认 supportsEdit=true 且 editUrl 已填写。";
      }

      const nodeLabel = buildImageNodeLabel(apiNode, index);
      const routeLabel = buildImageRouteLabel(routePlan);
      logger.info(`[图像任务] 收到修图请求 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} fallback=${usedFallback ? '是' : '否'} 图像路由=${routePlan.routerEnabled ? routeLabel : '未启用'} 提示词长度=${cleanPrompt.length} 提示词预览=${buildImagePromptPreview(cleanPrompt)} 选项=${buildImageOptionSummary(options, { sourceImageCount: imageBundle.images.length, currentImageCount: imageBundle.currentCount, quotedImageCount: imageBundle.quotedCount })}`);
      ...
      const routeWorkingNotice = routePlan.routerEnabled && routePlan.route.length > 1
        ? `图像路由集群：${routeLabel}`
        : `使用节点 #${index} ${apiNode.remark || apiNode.modelName}`;
      await sendAutoRecallImageNotice(session, `🖌️ 正在修图，${routeWorkingNotice}，输入图片 ${imageBundle.images.length} 张。\n${formatImageTimeoutHint()}，可在图像节点管理器调整超时时间。\n${IMAGE_NOTICE_RECALL_HINT}`);
      try {
        const result = await generateImagesWithRoute(ctx, routePlan, cleanPrompt, {
        ...
        const quotaUsage = recordImageQuotaUsage(session, 'edit', 1, access.isMasterUser, {
          modelName: result.model || successNode.modelName || getDefaultImageModelName(successNode),
          nodeRemark: successNode.remark || successNode.modelName,
          detail: { returnedAmount: Array.isArray(result.images) ? result.images.length : 0, sourceImageCount: imageBundle.images.length, routePath: result?.imageRoute?.path, failedRouteNodes: result?.imageRoute?.failures },
        });
        const sendStats = await sendXaiGeneratedImages(session, result.images, { actionLabel: '修图结果', nodeLabel: successNodeLabel });
        const imageCountNotice = Array.isArray(result.images) && result.images.length !== 1
          ? `\n⚠️ 接口本次返回 ${result.images.length} 张图。`
          : '';
        const modelFallbackNotice = buildImageModelFallbackNotice(result);
        const endpointFallbackNotice = buildImageEndpointFallbackNotice(result);
        const streamingNotice = buildImageStreamingNotice(result);
        const routeFallbackNotice = buildImageRouteFallbackNotice(result);
        const sendNotice = sendStats.failedCount > 0
          ? `\n⚠️ 实际发送成功 ${sendStats.sentCount} 张，发送失败 ${sendStats.failedCount} 张。`
          : '';
        logger.info(`[图像任务] 修图完成 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${successNodeLabel} 路由=${routePlan.routerEnabled ? routeLabel : '未启用'} 路由成功序号=${(result?.imageRoute?.usedRouteIndex ?? 0) + 1}/${result?.imageRoute?.total ?? 1} 返回张数=${Array.isArray(result.images) ? result.images.length : 0} 已发送=${sendStats.sentCount} 发送失败=${sendStats.failedCount} 无ACK=${sendStats.acklessCount} 模型=${result.model || successNode.modelName || getDefaultImageModelName(successNode)} 模型回退=${result?.modelFallback ? `${result.modelFallback.from}->${result.modelFallback.to}` : '否'} URL回退=${result?.endpointFallback ? `${result.endpointFallback.usedIndex + 1}/${result.endpointFallback.total}` : '否'}`);
        await sendAutoRecallImageNotice(session, `✅ 修图完成：节点 #${successNodeIndex} · ${result.model || successNode.modelName || getDefaultImageModelName(successNode)}${usedFallback && !routePlan.routerEnabled ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${routeFallbackNotice}${modelFallbackNotice}${endpointFallbackNotice}${streamingNotice}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
        logger.warn(`[图像任务] 修图失败 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} 路由=${routePlan.routerEnabled ? routeLabel : '未启用'} 输入图=${imageBundle.images.length} 提示词长度=${cleanPrompt.length} 提示词预览=${buildImagePromptPreview(cleanPrompt)} 错误=${e.message || '未知错误'}`);
        appendUsageEvent(session, {
          category: 'image',
          action: 'edit',
          allowed: false,
          ...
          modelName: apiNode.modelName,
          nodeRemark: apiNode.remark,
          detail: { error: e.message || '未知错误', sourceImageCount: imageBundle.images.length, routePath: routePlan.route.map((item) => buildImageNodeLabel(item.apiNode, item.index)) },
        });
