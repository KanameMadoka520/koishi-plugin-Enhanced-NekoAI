  ...
  ctx.command("neko.生图 <prompt:text>", "调用 xAI 图像生成接口生成图片")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      if (!access.isMasterUser && isBlacklisted(session.userId)) {
        logger.info(`[图像任务] 生图请求被黑名单拦截 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 提示词长度=${String(prompt || '').trim().length}`);
      ...
      if (!quotaCheck.allowed) {
        ...
        return buildImageQuotaExceededMessage(quotaCheck);
      }
      ...
      try {
        ...
        await session.send(`✅ 生图完成：节点 #${index} · ${result.model || apiNode.modelName || 'grok-imagine-image'}${usedFallback ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}`);
        return;
      } catch (e) {
  ...
  ctx.command("neko.修图 <prompt:text>", "附带图片或引用带图消息后，调用 xAI 图像编辑接口")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      try {
        ...
        await session.send(`✅ 修图完成：节点 #${index} · ${result.model || apiNode.modelName || 'grok-imagine-image'}${usedFallback ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}`);
        return;
      } catch (e) {
