const { formatCallFailure, safeModelName } = require('./chat-error');
/**
 * commands.js — 所有指令注册
...
async function generateImagesWithRoute(ctx, routePlan, prompt, options = {}, meta = {}) {
  ...
  for (let routeIndex = 0; routeIndex < route.length; routeIndex++) {
    ...
    try {
      ...
      lastRouteError = Object.assign(new Error(errMsg, { cause: error }), { modelName: error?.modelName || requestOptions.model || apiNode.modelName, status: error?.response?.status || error?.status, code: error?.code });
  ...
  ctx.command("neko.生图 <prompt:text>", "调用图像生成节点生成图片")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      const gptImage2Notice = isGptImage2Node(apiNode)
        ...
        : '';
      const routeWorkingNotice = `使用模型 ${safeModelName(apiNode.modelName)}`;
      ...
      try {
        ...
        await sendAutoRecallImageNotice(session, '❌ ' + formatCallFailure(e, e.modelName || apiNode.modelName));
        return;
      }
    });
