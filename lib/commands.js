    ...
    .action(async ({ session, options }, prompt) => {
      ...
      try {
        ...
        const quotaUsage = recordImageQuotaUsage(session, 'generate', actualGeneratedCount, access.isMasterUser, {
          ...
          detail: { requestedAmount: imageAmount, returnedAmount: Array.isArray(result.images) ? result.images.length : 0, sourceImageCount: referenceImages.length, routePath: result?.imageRoute?.path, failedRouteNodes: result?.imageRoute?.failures },
        });
        const sendStats = await sendXaiGeneratedImages(session, result.images, { actionLabel: '生图结果', nodeLabel: successNodeLabel });
        ...
        const sendNotice = sendStats.failedCount > 0
          ...
          : '';
        logger.info(`[图像任务] 生图完成 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${successNodeLabel} 路由=${routePlan.routerEnabled ? routeLabel : '未启用'} 路由成功序号=${(result?.imageRoute?.usedRouteIndex ?? 0) + 1}/${result?.imageRoute?.total ?? 1} 请求张数=${imageAmount} 返回张数=${Array.isArray(result.images) ? result.images.length : 0} 记账张数=${actualGeneratedCount} 已发送=${sendStats.sentCount} 发送失败=${sendStats.failedCount} 无ACK=${sendStats.acklessCount} 模型=${result.model || successNode.modelName || getDefaultImageModelName(successNode)} 模型回退=${result?.modelFallback ? `${result.modelFallback.from}->${result.modelFallback.to}` : '否'} URL回退=${result?.endpointFallback ? `${result.endpointFallback.usedIndex + 1}/${result.endpointFallback.total}` : '否'}`);
        await sendAutoRecallImageNotice(session, `✅ 生图完成：节点 #${successNodeIndex} · ${result.model || successNode.modelName || getDefaultImageModelName(successNode)}${usedFallback && !routePlan.routerEnabled ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${routeFallbackNotice}${modelFallbackNotice}${endpointFallbackNotice}${streamingNotice}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
        logger.warn(`[图像任务] 生图失败 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} 路由=${routePlan.routerEnabled ? routeLabel : '未启用'} 提示词长度=${cleanPrompt.length} 提示词预览=${buildImagePromptPreview(cleanPrompt)} 错误=${e.message || '未知错误'}`);
        appendUsageEvent(session, {
          category: 'image',
          action: 'generate',
          allowed: false,
          ...
          modelName: apiNode.modelName,
          nodeRemark: apiNode.remark,
          detail: { error: e.message || '未知错误', routePath: routePlan.route.map((item) => buildImageNodeLabel(item.apiNode, item.index)) },
        });
