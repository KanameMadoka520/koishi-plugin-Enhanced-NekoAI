  ...
  ctx.command("neko.生图 <prompt:text>", "调用图像生成节点生成图片")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      try {
        ...
        await sendAutoRecallImageNotice(session, `✅ 生图完成：节点 #${successNodeIndex} · ${result.model || successNode.modelName || getDefaultImageModelName(successNode)}${usedFallback && !routePlan.routerEnabled ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${routeFallbackNotice}${modelFallbackNotice}${endpointFallbackNotice}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
  ...
  ctx.command("neko.修图 <prompt:text>", "附带图片或引用带图消息后，调用图像编辑节点")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      try {
        ...
        await sendAutoRecallImageNotice(session, `✅ 修图完成：节点 #${successNodeIndex} · ${result.model || successNode.modelName || getDefaultImageModelName(successNode)}${usedFallback && !routePlan.routerEnabled ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${routeFallbackNotice}${modelFallbackNotice}${endpointFallbackNotice}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
