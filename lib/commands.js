...
function imageNodeSupportsEdit(apiNode) {
  ...
  if (provider === 'openai' && isGptImage2Model(apiNode?.modelName || getDefaultImageModelName(apiNode))) return false;
...
说明：
${buildXaiImageOptionHintText()}
修图命令需要当前消息附图，或引用一条带图消息。gpt-image-2 图像节点仅支持生图，修图会自动跳过这类节点；若不写 --ratio / --resolution，则优先走当前图像节点的默认值。`;
}
  ...
  ctx.command("neko.修图 <prompt:text>", "附带图片或引用带图消息后，调用图像编辑节点")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      if (!apiNode) {
        ...
        return "❌ 未找到可用的图像编辑节点。请在 image_api_config.json 或 GUI 的图像节点列表中配置支持修图的图像节点。注意：gpt-image-2 图像节点仅支持生图，不会被修图命令使用。";
      }
