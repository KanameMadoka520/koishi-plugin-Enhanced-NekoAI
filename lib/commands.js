...
function registerCommands(ctx) {
  ...
  ctx.command("neko.生图 <prompt:text>", "调用图像生成节点生成图片")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      const referenceBundle = imageNodeSupportsEdit(apiNode)
      ...
      logger.info(`[图像任务] 收到生图请求 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} fallback=${usedFallback ? '是' : '否'} 提示词长度=${cleanPrompt.length} 提示词预览=${buildImagePromptPreview(cleanPrompt)} 选项=${buildImageOptionSummary(options, { requestedCount: imageAmount, sourceImageCount: referenceImages.length, currentImageCount: referenceBundle.currentCount, quotedImageCount: referenceBundle.quotedCount })}`);
      ...
      const gptImage2Notice = isGptImage2Node(apiNode)
        ...
        : '';
      await sendAutoRecallImageNotice(session, `🎨 正在生成图片，使用节点 #${index} ${apiNode.remark || apiNode.modelName}。${referenceNotice}${gptImage2Notice}\n${formatImageTimeoutHint()}，可在图像节点管理器调整超时时间。\n${IMAGE_NOTICE_RECALL_HINT}`);
      try {
        const result = await generateImages(ctx, apiNode, cleanPrompt, {
          n: options?.count,
        ...
        const quotaUsage = recordImageQuotaUsage(session, 'generate', actualGeneratedCount, access.isMasterUser, {
          modelName: result.model || apiNode.modelName || getDefaultImageModelName(apiNode),
          nodeRemark: apiNode.remark || apiNode.modelName,
          detail: { requestedAmount: imageAmount, returnedAmount: Array.isArray(result.images) ? result.images.length : 0, sourceImageCount: referenceImages.length },
        });
        ...
        const sendNotice = sendStats.failedCount > 0
          ...
          : '';
        logger.info(`[图像任务] 生图完成 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} 请求张数=${imageAmount} 返回张数=${Array.isArray(result.images) ? result.images.length : 0} 记账张数=${actualGeneratedCount} 已发送=${sendStats.sentCount} 发送失败=${sendStats.failedCount} 无ACK=${sendStats.acklessCount} 模型=${result.model || apiNode.modelName || getDefaultImageModelName(apiNode)} 模型回退=${result?.modelFallback ? `${result.modelFallback.from}->${result.modelFallback.to}` : '否'} URL回退=${result?.endpointFallback ? `${result.endpointFallback.usedIndex + 1}/${result.endpointFallback.total}` : '否'}`);
        await sendAutoRecallImageNotice(session, `✅ 生图完成：节点 #${index} · ${result.model || apiNode.modelName || getDefaultImageModelName(apiNode)}${usedFallback ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${modelFallbackNotice}${endpointFallbackNotice}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      try {
        ...
        const sendNotice = sendStats.failedCount > 0
          ? `\n⚠️ 实际发送成功 ${sendStats.sentCount} 张，发送失败 ${sendStats.failedCount} 张。`
          : '';
        logger.info(`[图像任务] 修图完成 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} 返回张数=${Array.isArray(result.images) ? result.images.length : 0} 已发送=${sendStats.sentCount} 发送失败=${sendStats.failedCount} 无ACK=${sendStats.acklessCount} 模型=${result.model || apiNode.modelName || getDefaultImageModelName(apiNode)} 模型回退=${result?.modelFallback ? `${result.modelFallback.from}->${result.modelFallback.to}` : '否'} URL回退=${result?.endpointFallback ? `${result.endpointFallback.usedIndex + 1}/${result.endpointFallback.total}` : '否'}`);
        await sendAutoRecallImageNotice(session, `✅ 修图完成：节点 #${index} · ${result.model || apiNode.modelName || getDefaultImageModelName(apiNode)}${usedFallback ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${modelFallbackNotice}${endpointFallbackNotice}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
