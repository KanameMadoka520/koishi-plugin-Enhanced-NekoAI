...
const { generateImages, getImageApiTimeoutMs } = require('./api');
...
function hasUsableXaiImageNode(apiNode, mode) {
  ...
  if (mode === 'edit') return imageNodeSupportsEdit(apiNode) && !!String(apiNode?.editUrl || '').trim();
  ...
  ctx.command("neko.修图 <prompt:text>", "附带图片或引用带图消息后，调用图像编辑节点")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      try {
        ...
        await sendAutoRecallImageNotice(session, `✅ 修图完成：节点 #${index} · ${result.model || apiNode.modelName || getDefaultImageModelName(apiNode)}${usedFallback ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${modelFallbackNotice}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
