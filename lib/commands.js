...
const { isMaster, isBlacklisted, loadCommandsList, updateGroupFriends, getPeriodInfo, getGroupPersonalityIndex, getGroupApiIndex, getXaiWebSearchNotice, isKnownGroupFriend } = require('./utils');
...
function registerCommands(ctx) {
  ...
  ctx.command("neko.生图 <prompt:text>", "调用 xAI 图像生成接口生成图片")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      if (!access.isMasterUser && isBlacklisted(session.userId)) {
        return '❌ 黑名单用户不允许使用生图功能。';
      }
      ...
      try {
        ...
        const quotaUsage = recordImageQuotaUsage(session, 'generate', imageAmount, access.isMasterUser);
