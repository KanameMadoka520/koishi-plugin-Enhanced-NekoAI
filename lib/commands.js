...
const { isMaster, loadCommandsList, updateGroupFriends, getPeriodInfo, getGroupPersonalityIndex, getGroupApiIndex, getXaiWebSearchNotice, isKnownGroupFriend } = require('./utils');
...
const { checkImageQuota, recordImageQuotaUsage, buildImageQuotaExceededMessage } = require('./ratelimit');
...
function canUseImageCommand(session) {
  if (isMaster(session)) return { allowed: true, isMasterUser: true };
  if (!session?.isDirect) return { allowed: true, isMasterUser: false };
  if (isKnownGroupFriend(session?.userId)) return { allowed: true, isMasterUser: false };
  return {
    allowed: false,
    isMasterUser: false,
    message: '❌ 当前图像指令仅支持主人或群友使用。若你是群友但刚被放进新的监听群，请稍后让主人执行一次群友名单刷新。',
  };
}
...
function registerCommands(ctx) {
  ...
  ctx.command("neko.生图 <prompt:text>", "调用 xAI 图像生成接口生成图片")
    .option('count', '--count <count:number>', { fallback: 1 })
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      if (!quotaCheck.allowed) return buildImageQuotaExceededMessage(quotaCheck);
      ...
      try {
        ...
        recordImageQuotaUsage(session, 'generate', imageAmount, access.isMasterUser);
        await sendXaiGeneratedImages(session, result.images);
        await session.send(`✅ 生图完成：节点 #${index} · ${result.model || apiNode.modelName || 'grok-imagine-image'}${usedFallback ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}`);
        return;
      } catch (e) {
  ...
  ctx.command("neko.修图 <prompt:text>", "附带图片或引用带图消息后，调用 xAI 图像编辑接口")
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      try {
        ...
        recordImageQuotaUsage(session, 'edit', 1, access.isMasterUser);
        await sendXaiGeneratedImages(session, result.images);
        await session.send(`✅ 修图完成：节点 #${index} · ${result.model || apiNode.modelName || 'grok-imagine-image'}${usedFallback ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}`);
        return;
      } catch (e) {
