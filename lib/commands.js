    ...
    .action(async ({ session, options }, prompt) => {
      ...
      try {
        ...
        const sendNotice = sendStats.failedCount > 0
          ...
          : '';
        logger.info(`[图像任务] 生图完成 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} 请求张数=${imageAmount} 返回张数=${Array.isArray(result.images) ? result.images.length : 0} 记账张数=${actualGeneratedCount} 已发送=${sendStats.sentCount} 发送失败=${sendStats.failedCount} 无ACK=${sendStats.acklessCount} 模型=${result.model || apiNode.modelName || 'grok-imagine-image'} 模型回退=${result?.modelFallback ? `${result.modelFallback.from}->${result.modelFallback.to}` : '否'}`);
        await sendAutoRecallImageNotice(session, `✅ 生图完成：节点 #${index} · ${result.model || apiNode.modelName || 'grok-imagine-image'}${usedFallback ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${modelFallbackNotice}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
    ...
    .action(async ({ session, options }, prompt) => {
      ...
      try {
        ...
        const sendNotice = sendStats.failedCount > 0
          ? `\n⚠️ 实际发送成功 ${sendStats.sentCount} 张，发送失败 ${sendStats.failedCount} 张。`
          : '';
        logger.info(`[图像任务] 修图完成 用户=${session.userId} 场景=${buildImageSceneLabel(session)} 节点=${nodeLabel} 返回张数=${Array.isArray(result.images) ? result.images.length : 0} 已发送=${sendStats.sentCount} 发送失败=${sendStats.failedCount} 无ACK=${sendStats.acklessCount} 模型=${result.model || apiNode.modelName || 'grok-imagine-image'} 模型回退=${result?.modelFallback ? `${result.modelFallback.from}->${result.modelFallback.to}` : '否'}`);
        await sendAutoRecallImageNotice(session, `✅ 修图完成：节点 #${index} · ${result.model || apiNode.modelName || 'grok-imagine-image'}${usedFallback ? '（默认图像节点不可用，已自动改用首个可用图像节点）' : ''}${modelFallbackNotice}${imageCountNotice}${sendNotice}\n${buildImageQuotaUsageNotice({ ...quotaUsage, isMasterUser: access.isMasterUser })}\n${IMAGE_NOTICE_RECALL_HINT}`);
        return;
      } catch (e) {
