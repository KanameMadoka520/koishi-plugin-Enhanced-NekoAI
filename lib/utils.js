...
function getChatAccessConfig() {
  const raw = state.runtimeConfig.chatAccess && typeof state.runtimeConfig.chatAccess === 'object'
    ? state.runtimeConfig.chatAccess
    : {};
  return {
    mode: raw.mode === 'whitelist' ? 'whitelist' : 'blacklist',
    whitelistUsers: normalizeUserIdList(raw.whitelistUsers),
...
function getChatAccessConflictUsers() {
  const blacklist = normalizeUserIdList(state.runtimeConfig.userBlacklist);
  const whitelist = getChatAccessConfig().whitelistUsers;
...
module.exports = {
  ...
  isKnownGroupFriend, getChatAccessConfig, getChatAccessConflictUsers, getImageAccessConfig, getImageAccessConflictUsers,
