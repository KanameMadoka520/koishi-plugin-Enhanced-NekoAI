...
function buildXaiImageConfig(apiNode) {
  return {
    enabled: apiNode?.xaiImageEnabled === true,
    model: String(apiNode?.xaiImageModel || '').trim() || 'grok-imagine-image',
    generationUrl: String(apiNode?.xaiImageGenerationUrl || '').trim(),
    editUrl: String(apiNode?.xaiImageEditUrl || '').trim(),
    aspectRatio: String(apiNode?.xaiImageAspectRatio || '').trim(),
    resolution: String(apiNode?.xaiImageResolution || '').trim(),
  };
}
...
async function generateXaiImages(ctx, apiNode, prompt, options = {}) {
  ...
  if (!config.enabled) {
    throw new Error('当前节点未启用 xAI 图像能力');
  }
  ...
  if (config.aspectRatio && (!isEdit || sourceImages.length > 1)) payload.aspect_ratio = config.aspectRatio;
  ...
  const data = await postXaiImageRequest(ctx, requestUrl, payload, headers);
