...
async function callApiOnce(ctx, apiNode, historyTextArray, systemPrompt, currentImages) {
  ...
  const aiType = (apiNode.aiType || "openai").toLowerCase();
  ...
  const data = await ctx.http.post(requestUrl, payload, { headers, timeout: 120000 });

  // 解析响应
  let content = "skip";
  if (aiType === "anthropic") {
    if (data && data.content && data.content.length > 0) content = data.content[0].text;
  } else if (aiType === "gemini") {
...
async function getAiReply(ctx, historyTextArray, systemPrompt, currentImages, overrideApiIndex) {
  ...
  for (let attempt = 0; attempt < indicesToTry.length; attempt++) {
    ...
    const aiType = (currentApi.aiType || "openai").toLowerCase();
    ...
    try {
      ...
      if (currentImages && currentImages.length > 0 && !isRetryableError(err)) {
        ...
        return { reply: "无法处理此图片，请更换具有视觉理解的模型或者检查后台日志。", emoji: "尴尬", origin: "error", errorMsg: "图片处理失败", apiInfo, responseTime };
      }

      if (!router.enabled || !isRetryableError(err)) {
        // 路由关闭或不可重试的错误，直接返回
        if (router.enabled) logger.warn(`错误不可重试(${err?.response?.status || '未知状态码'})，停止尝试`);
        return { reply: "skip", emoji: "skip", origin: "error", errorMsg: err.message, apiInfo, responseTime };
      }
  ...
  // 所有节点都失败了
  const responseTime = Date.now() - t0;
  logger.error(`所有 ${indicesToTry.length} 个节点均失败，最后错误: ${lastError?.message}`);
  return { reply: "skip", emoji: "skip", origin: "error", errorMsg: lastError?.message || "所有节点失败", apiInfo: { modelName: "all-failed", remark: "all-failed" }, responseTime };
}
