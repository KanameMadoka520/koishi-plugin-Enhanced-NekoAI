...
async function generateOpenAiImages(ctx, apiNode, prompt, options = {}) {
  ...
  logger.info(`[OpenAI图像请求] 模式=${isEdit ? '修图' : '生图'} 节点=${apiNode?.remark || apiNode?.modelName || '未命名节点'} URL=${sanitizeUrlForLog(requestUrl)} 模型=${resolvedModel} n=${isEdit ? '-' : imageCount} ratio=${resolvedAspectRatio || '[未设]'} resolution=${resolvedResolution || '[未设]'} 输入图=${sourceImages.length} 提示词长度=${cleanPrompt.length}`);
  ...
  try {
    ...
    const errorInfo = buildImageRequestErrorInfo(err);
    logger.warn(`[OpenAI图像请求失败] 模式=${isEdit ? '修图' : '生图'} 节点=${apiNode?.remark || apiNode?.modelName || '未命名节点'} URL=${sanitizeUrlForLog(requestUrl)} 模型=${resolvedModel} n=${isEdit ? '-' : imageCount} ratio=${resolvedAspectRatio || '[未设]'} resolution=${resolvedResolution || '[未设]'} 输入图=${sourceImages.length} ${errorInfo.logMessage}`);
    throw new Error(errorInfo.publicMessage);
  }
  ...
  logger.info(`[OpenAI图像响应] 模式=${isEdit ? '修图' : '生图'} 节点=${apiNode?.remark || apiNode?.modelName || '未命名节点'} 原始图片=${rawImageCount} 归一化后=${images.length}`);
  if (!isEdit && imageCount > 1 && images.length > 0 && images.length < imageCount) {
...
module.exports = { getAiReply, generateXaiImages, generateOpenAiImages, generateImages, getImageApiTimeoutMs };
