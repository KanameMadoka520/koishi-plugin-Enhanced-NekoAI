...
/**
 * [新增] 提取回复引用的原始消息内容
 * 当用户回复某条消息并 @AI 时，提取被引用消息的文本
 */
async function extractQuotedContent(session) {
  try {
      ...
      if (msg) {
        ...
        if (msg.elements && msg.elements.length > 0) {
          for (const el of msg.elements) {
            if (el.type === 'text') quotedText += el.attrs.content;
            else if (el.type === 'img' || el.type === 'image') quotedText += " [图片]";
          }
        } else if (msg.content) {
          quotedText = msg.content.replace(/<[^>]+>/g, "").trim();
        }

        if (quotedText) {
          const senderName = msg.author?.name || msg.author?.username || msg.author?.nickname || msg.userId || "某人";
          logger.debug(`提取到引用消息: [${senderName}] ${quotedText.substring(0, 50)}...`);
          return { senderName, text: quotedText };
        }
      }
    } catch (e) {
