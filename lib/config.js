...
const { USAGE_EVENT_SCHEMA_VERSION, sanitizeUsageEventStore } = require('./usage-events');
...
const defaultRuntimeConfig = {
  "_comments": {
    ...
    "chatAccess": "聊天权限配置：控制普通聊天回复的权限模式与聊天白名单",
    "chatQuota": "聊天限额配置：控制普通聊天默认额度与指定QQ的单独额度",
    "imageAccess": "图像权限配置：控制生图/修图的权限模式与图像白名单",
    "imageQuota": "图像限额配置：控制生图/修图默认额度与指定QQ的单独额度",
    "apiTimeoutMs": "调用下游聊天 API 的超时时间(ms)",
    "imageApiTimeoutMs": "调用下游图像 API 的超时时间(ms)，控制生图/修图最长等待时间",
    ...
    "imageRouter": "图像路由集群配置：按指定图像节点顺序接力尝试",
...
const defaultImageApiConfig = [
  { "providerType": "xai", "generationUrl": "https://api.x.ai/v1/images/generations", "generationUrls": [], "editUrl": "https://api.x.ai/v1/images/edits", "editUrls": [], "apiKey": "xai-your-key", "modelName": "grok-imagine-image", "remark": "xAI 官方图像示例", "aspectRatio": "", "resolution": "", "supportsEdit": true },
  { "providerType": "openai", "generationUrl": "https://api.openai.com/v1/images/generations", "generationUrls": [], "editUrl": "https://api.openai.com/v1/images/edits", "editUrls": [], "apiKey": "sk-your-openai-key", "modelName": "gpt-image-2", "remark": "OpenAI 图像示例", "aspectRatio": "", "resolution": "", "supportsEdit": true, "streamingEnabled": false, "partialImages": 2, "streamingFallbackToNonStream": true }
];
...
function loadAllConfigs() {
  ...
  if (!fs.existsSync(runPath)) {
    ...
    try {
      ...
      // 深层补丁：smartRouter / imageRouter / apiParams / memorySummary / requestQueue / chatAccess / chatQuota / imageAccess / imageQuota / _comments
      for (const deepKey of ['smartRouter', 'imageRouter', 'apiParams', 'memorySummary', 'requestQueue', 'chatAccess', 'chatQuota', 'imageAccess', 'imageQuota', '_comments']) {
      ...
      if (!state.runtimeConfig.chatAccess || typeof state.runtimeConfig.chatAccess !== 'object' || Array.isArray(state.runtimeConfig.chatAccess)) {
        state.runtimeConfig.chatAccess = JSON.parse(JSON.stringify(defaultRuntimeConfig.chatAccess));
        updated = true;
      }
      if (!state.runtimeConfig.chatQuota || typeof state.runtimeConfig.chatQuota !== 'object' || Array.isArray(state.runtimeConfig.chatQuota)) {
        state.runtimeConfig.chatQuota = JSON.parse(JSON.stringify(defaultRuntimeConfig.chatQuota));
        updated = true;
      }
      if (!state.runtimeConfig.imageAccess || typeof state.runtimeConfig.imageAccess !== 'object' || Array.isArray(state.runtimeConfig.imageAccess)) {
        state.runtimeConfig.imageAccess = JSON.parse(JSON.stringify(defaultRuntimeConfig.imageAccess));
        updated = true;
      }
      if (!state.runtimeConfig.imageQuota || typeof state.runtimeConfig.imageQuota !== 'object' || Array.isArray(state.runtimeConfig.imageQuota)) {
        state.runtimeConfig.imageQuota = JSON.parse(JSON.stringify(defaultRuntimeConfig.imageQuota));
        updated = true;
      }
  ...
  if (!fs.existsSync(usagePath)) {
    state.usageData = { periodId: currentPeriod, counts: {}, users: {} };
    ...
    try {
      ...
      if (!state.usageData || typeof state.usageData !== 'object' || Array.isArray(state.usageData)) {
        state.usageData = { periodId: currentPeriod, counts: {}, users: {} };
      }
    ...
    } catch (e) { state.usageData = { periodId: currentPeriod, counts: {}, users: {} }; }
  }
  ...
  // 5.8 用量事件日志
  const usageEventsPath = path.join(BASE_DIR, FILES.USAGE_EVENTS);
  if (!fs.existsSync(usageEventsPath)) {
    state.usageEventData = { schemaVersion: USAGE_EVENT_SCHEMA_VERSION, events: [] };
    fs.writeFileSync(usageEventsPath, JSON.stringify(state.usageEventData, null, 2), "utf-8");
  } else {
    try {
      const raw = JSON.parse(fs.readFileSync(usageEventsPath, "utf-8"));
      const result = sanitizeUsageEventStore(raw);
      state.usageEventData = result.store;
      if (result.changed) {
        saveUsageEvents();
        const patches = [];
        if (result.droppedCount > 0) patches.push(`移除了 ${result.droppedCount} 条无效事件`);
        if (result.normalizedCount > 0) patches.push(`规范化了 ${result.normalizedCount} 条历史事件`);
        if (result.prunedCount > 0) patches.push(`裁剪了 ${result.prunedCount} 条过旧事件`);
        if (patches.length > 0) {
          logger.warn(`usage_events.json 已自动整理：${patches.join('，')}`);
        }
      }
    } catch (e) {
      state.usageEventData = { schemaVersion: USAGE_EVENT_SCHEMA_VERSION, events: [] };
      saveUsageEvents();
      logger.warn(`读取 usage_events.json 失败，已重置为空日志：${e?.message || '未知错误'}`);
    }
  }
...
function saveUsageEvents() {
  try { fs.writeFileSync(path.join(BASE_DIR, FILES.USAGE_EVENTS), JSON.stringify(state.usageEventData, null, 2), "utf-8"); }
  catch (e) { logger.error("保存用量事件日志失败", e.message); }
}

module.exports = {
  name, usage, Config, FILES, BASE_DIR, defaultRuntimeConfig,
  loadAllConfigs, saveRuntimeConfig, saveApiConfig,
  saveImageApiConfig, saveGroupPersonality, savePrivatePersonality, saveUsageCounts, saveImageUsageCounts, saveUsageEvents
};
