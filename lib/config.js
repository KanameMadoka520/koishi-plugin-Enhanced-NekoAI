...
function loadAllConfigs() {
  ...
  if (!fs.existsSync(usageEventsPath)) {
    state.usageEventData = { schemaVersion: 1, events: [] };
    ...
    try {
      ...
      if (Array.isArray(raw)) {
        state.usageEventData = { schemaVersion: 1, events: raw };
        saveUsageEvents();
      } else {
        state.usageEventData = raw && typeof raw === 'object' ? raw : { schemaVersion: 1, events: [] };
        ...
        if (!Number.isInteger(Number(state.usageEventData.schemaVersion))) {
          state.usageEventData.schemaVersion = 1;
          saveUsageEvents();
        }
      }
    } catch (e) {
