...
function recordImageQuotaUsage(session, action, amount = 1, isMasterUser = false, meta = {}) {
  ...
  if (isMasterUser) {
    ...
    return {
      ...
      action,
      actionLabel: action === 'generate' ? '生图' : '修图',
      amount: normalizedAmount,
      unlimited: true,
      limit: null,
      used: 0,
...
function buildImageQuotaUsageNotice(result) {
  if (!result) return '';
  const actionLabel = result.actionLabel || (result.action === 'edit' ? '修图' : '生图');
  ...
  const used = normalizeQuotaNumber(result.used);
  const rawLimit = Number(result.limit);
  const rawRemaining = Number(result.remaining);
  let limit = Number.isFinite(rawLimit) && rawLimit > 0 ? Math.floor(rawLimit) : null;
  let remaining = Number.isFinite(rawRemaining) && rawRemaining >= 0 ? Math.floor(rawRemaining) : null;

  if (limit === null && remaining !== null) limit = used + remaining;
  if (remaining === null && limit !== null) remaining = Math.max(limit - used, 0);

  return `本周期已用${actionLabel}额度：${used}/${limit ?? '未知'}\n本周期剩余${actionLabel}额度：${remaining ?? '未知'}\n下次刷新：${getNextPeriodResetText()}`;
}
