...
const PROCESSING_NOTICE_RECALL_MS = 30000;

function scheduleProcessingNoticeRecall(session, messageIds) {
  ...
  setTimeout(async () => {
    for (const id of ids) {
      ...
      try {
        ...
        logger.debug(`撤回处理中提示失败(${id}): ${e.message}`);
      }
    }
  }, PROCESSING_NOTICE_RECALL_MS);
}

function createProcessingNoticeTask(session) {
  ...
  async function sendNow() {
    ...
    sent = true;
    try {
      const messageIds = await safeReplyOrNotify(session, cfg.text);
      scheduleProcessingNoticeRecall(session, messageIds);
    } catch (e) {
  ...
  ctx.on("message", async (session) => {
    ...
    if (session.isDirect) {
      ...
      state.privateIntervals[uid] = setTimeout(async () => {
        ...
        const noticeTask = createProcessingNoticeTask(session);
        noticeTask.start();
        ...
        if (result.origin !== "skip" && result.origin !== "error") state.singleMessages[uid].push(SerializeMessage(state.runtimeConfig.nickName, result.origin));
        ...
        if (state.receive[gid] === true) {
          if (Math.random() < state.runtimeConfig.randomReply) {
            ...
            try {
              ...
              noticeTask.cancel();
              if (result.origin !== "skip" && result.origin !== "error") {
...
async function handleGroupMention(ctx, session, currentText, currentImages) {
  ...
  logger.info(`[群聊@] 发送者[${senderName}] 专注模式[${mentionFocusMode ? '开' : '关'}] 当前消息[${normalizedText.length}字] 当前图片[${currentImages.length}张] 引用[${quoted ? '有' : '无'}]${quoted ? ` 引用发送者[${quoted.senderName}] 引用文本[${String(quoted.rawText || '').length}字] 引用图片[${quoted.resolvedImageCount || 0}/${quoted.imageCount || 0}张]` : ''}`);
