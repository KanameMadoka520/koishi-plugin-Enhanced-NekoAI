const { redactForChat, rawErrorText } = require('./chat-error');
/**
 * listener.js — 消息事件处理
...
const { isMaster, isBlacklisted, isKoishiCommand, getGroupPersonalityIndex, getGroupApiIndex, safeReplyOrNotify, updateGroupFriends, getXaiWebSearchNotice, getApiNodeByIndex, getChatAccessConfig } = require('./utils');
...
const { checkAndUpdateGroupLimit, buildChatQuotaExceededMessage } = require('./ratelimit');
...
function buildProcessingNoticeText(snapshot, baseText, apiNode) {
  const cleanedBaseText = stripRecallHint(baseText) || 'NEKOAI已接收到请求，正在处理中。';
  const webSearchNotice = getXaiWebSearchNotice(apiNode, '当前模型');
  const webSearchLine = webSearchNotice ? `${webSearchNotice}\n` : '';
  if (snapshot.willQueue) {
    return `NEKOAI请求并发已满，正在排队处理。\n${webSearchLine}当前前方有${snapshot.ahead}个请求未完成（运行中 ${snapshot.running}/${snapshot.maxConcurrent}，排队 ${snapshot.pending}/${formatPendingLimit(snapshot)}）。\n${REPEAT_REQUEST_HINT}\n${NOTICE_RECALL_HINT}`;
  }
  ...
  return `${cleanedBaseText}\n${webSearchLine}当前并发 ${projectedRunning}/${snapshot.maxConcurrent}，排队 ${snapshot.pending}/${formatPendingLimit(snapshot)}。\n${REPEAT_REQUEST_HINT}\n${NOTICE_RECALL_HINT}`;
}
...
function createProcessingNoticeTask(session, queueSnapshot, apiNode) {
  ...
  async function sendNow() {
    ...
    sent = true;
    try {
      const text = buildProcessingNoticeText(queueSnapshot, cfg.text, apiNode);
      const messageIds = await safeReplyOrNotify(session, text);
...
function buildGenerationFailedMessage(result) {
  const cfg = getFailureNoticeConfig();
  if (!cfg.enabled) return '';
  const nodes = [...(state.apiList || []), ...(state.imageApiList || []), result?.apiInfo || {}];
  const info = result?.apiInfo || {};
  const label = (Number.isInteger(info.nodeIndex) ? '节点 #' + info.nodeIndex + ' · ' : '模型：') + (info.modelName || 'Unknown model');
  const detail = result?.error ? rawErrorText(result.error) : (result?.errorMsg || 'Unknown error');
  return redactForChat('❌ ' + label + ' 请求失败：\n' + detail + '\n' + cfg.retryText, nodes);
}
...
function canUseChatConversation(session) {
  const userId = String(session?.userId || '');
  const access = getChatAccessConfig();
  if (isMaster(session)) {
    return { allowed: true, isMasterUser: true, accessMode: access.mode, reason: 'master' };
  }
  if (access.mode === 'whitelist' && !access.whitelistUsers.includes(userId)) {
    return {
      allowed: false,
      isMasterUser: false,
      accessMode: 'whitelist',
      reason: 'whitelist-required',
      message: '❌ 当前聊天权限为白名单模式，只有被手动加入聊天白名单的 QQ 才能获得普通聊天回复。',
    };
  }
  return {
    allowed: true,
    ...
    reason: access.mode === 'whitelist' ? 'whitelist-allowed' : 'blacklist-mode',
  };
}
...
function registerListener(ctx) {
  ...
  ctx.on("message", async (session) => {
    ...
    if ((session.content.includes(state.runtimeConfig.nickName) || session.content.includes(session.bot.selfId)) &&
      ...
      const chatAccess = canUseChatConversation(session);
      if (!chatAccess.allowed) {
        logger.info(`[聊天权限] 群聊@请求被拦截 用户=${session.userId} 群=${session.channelId} 模式=${chatAccess.accessMode || 'unknown'} 原因=${chatAccess.reason || 'unknown'}`);
        appendUsageEvent(session, {
          category: 'chat',
          action: 'chat',
          allowed: false,
          amount: 1,
          reason: chatAccess.reason || 'permission-denied',
          isMasterUser: false,
        });
        await safeReplyOrNotify(session, chatAccess.message);
        return;
      }
    ...
    if (session.isDirect) {
      const chatAccess = canUseChatConversation(session);
      if (!chatAccess.allowed) {
        logger.info(`[聊天权限] 私聊请求被拦截 用户=${session.userId} 模式=${chatAccess.accessMode || 'unknown'} 原因=${chatAccess.reason || 'unknown'}`);
        appendUsageEvent(session, {
          category: 'chat',
          action: 'chat',
          allowed: false,
      ...
      if (state.privateMode === 'master') {
        appendUsageEvent(session, { category: 'chat', action: 'chat', allowed: false, amount: 1, reason: 'private-mode-master', isMasterUser: false });
        await sleep(1000); await safeReplyOrNotify(session, state.runtimeConfig.privateRefuse); return;
      }
      else if (state.privateMode === 'friends' && !isFriend) {
        appendUsageEvent(session, { category: 'chat', action: 'chat', allowed: false, amount: 1, reason: 'private-mode-friends-only', isMasterUser: false });
        await sleep(1000); await safeReplyOrNotify(session, "拒绝非群友私聊"); return;
      }
    }
      ...
      state.privateIntervals[uid] = setTimeout(async () => {
        const limitResult = await checkAndUpdateGroupLimit(session, isMaster(session), { notifyThresholds: false });
        if (!limitResult.allowed) {
          await safeReplyOrNotify(session, buildChatQuotaExceededMessage(limitResult));
        ...
        const apiNode = getApiNodeByIndex(state.runtimeConfig.activeApiIndex || 0);
        const noticeTask = createProcessingNoticeTask(session, queueSnapshot, apiNode);
      ...
      if (state.messageCount[gid] >= state.runtimeConfig.messagesLength) {
        ...
        if (state.receive[gid] === true) {
          if (Math.random() < state.runtimeConfig.randomReply) {
            ...
            const chatAccess = canUseChatConversation(session);
            if (!chatAccess.allowed) {
              logger.debug(`[聊天权限] 被动插嘴因权限限制被跳过 用户=${session.userId} 群=${gid} 模式=${chatAccess.accessMode || 'unknown'} 原因=${chatAccess.reason || 'unknown'}`);
              return;
            }

            // 限流检测（被动插嘴静默拦截）
            const limitResult = await checkAndUpdateGroupLimit(session, false, { notifyThresholds: true });
            ...
            try {
              ...
              const apiNode = getApiNodeByIndex(apiIdx);
              const noticeTask = createProcessingNoticeTask(session, queueSnapshot, apiNode);
              noticeTask.start(queueSnapshot.willQueue);
...
async function handleGroupMention(ctx, session, currentText, currentImages) {
  ...
  // 限流检测
  let limitResult = await checkAndUpdateGroupLimit(session, isMaster(session), { notifyThresholds: true });
  if (!limitResult.allowed) {
    await session.send(buildChatQuotaExceededMessage(limitResult));
    return;
  }
