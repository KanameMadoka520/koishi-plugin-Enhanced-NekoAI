        ...
        if (state.receive[gid] === true) {
          if (Math.random() < state.runtimeConfig.randomReply) {
            ...
            try {
              ...
              noticeTask.cancel();
              if (result.origin !== "skip" && result.origin !== "error") state.historyMessages[gid].push(SerializeMessage(state.runtimeConfig.nickName, result.origin));
              await sendAiResultToChat(session, result);
...
async function handleGroupMention(ctx, session, currentText, currentImages) {
  ...
  await sendAiResultToChat(session, result);
  if (result.origin !== "skip" && result.origin !== "error") state.historyMessages[session.channelId].push(SerializeMessage(state.runtimeConfig.nickName, result.origin));
