const { formatCallFailure } = require('./chat-error');
/**
 * listener.js — 消息事件处理
...
function buildGenerationFailedMessage(result) {
  if (!getFailureNoticeConfig().enabled) return '';
  return '❌ ' + formatCallFailure(result?.error || result?.errorMsg, result?.apiInfo?.modelName);
}
